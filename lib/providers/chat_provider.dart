...
import '../models/chat_model.dart' as chat;
import '../models/chat_message_model.dart' as msg;
...
class ChatProvider extends ChangeNotifier {
  ...
  // Chat conversations
  List<chat.ChatModel> _conversations = [];
  chat.ChatModel? _currentConversation;
  
  // Messages for current conversation
  List<msg.ChatMessageModel> _messages = [];
  
  // Typing indicators
  Map<String, List<msg.TypingIndicator>> _typingIndicators = {};
  ...
  // Getters
  List<chat.ChatModel> get conversations => List.unmodifiable(_conversations);
  chat.ChatModel? get currentConversation => _currentConversation;
  List<msg.ChatMessageModel> get messages => List.unmodifiable(_messages);
  Map<String, List<msg.TypingIndicator>> get typingIndicators => Map.unmodifiable(_typingIndicators);
  ...
  Future<String> createConversation(String ashaId, String ashaName, {String? ashaImageUrl}) async {
    ...
    try {
      ...
      final conversationData = {
        ...
        'type': chat.ChatType.individual.toString(),
        'isActive': true,
  ...
  Future<void> _loadConversations() async {
    ...
    try {
      ...
      _conversations = snapshot.docs.map((doc) => chat.ChatModel.fromFirestore(doc)).toList();
      notifyListeners();
  ...
  Future<void> _loadMessages(String conversationId) async {
    try {
      ...
      _messages = snapshot.docs
          .map((doc) => msg.ChatMessageModel.fromFirestore(doc))
          .toList()
          .reversed
          .toList();
  ...
  void _listenToMessages(String conversationId) {
    ...
    _messagesListener = _firestore
        ...
        .listen(
      (snapshot) {
        final newMessages = <msg.ChatMessageModel>[];
        for (final docChange in snapshot.docChanges) {
          final message = msg.ChatMessageModel.fromFirestore(docChange.doc);
  ...
  Future<void> sendMessage(String content, {String? replyToMessageId}) async {
    ...
    try {
      ...
      final messageData = {
        ...
        'type': msg.MessageType.text.toString(),
        'content': content,
        'timestamp': Timestamp.fromDate(now),
        'readBy': [_currentUserId!],
        'status': msg.MessageStatus.sent.toString(),
        'isEdited': false,
        'isDeleted': false,
  ...
  Future<void> sendImageMessage(String imageUrl, {String? caption}) async {
    ...
    try {
      ...
      final messageData = {
        ...
        'senderName': _currentUserName,
        'senderRole': _currentUserRole,
        'type': msg.MessageType.image.toString(),
        'content': caption ?? '',
        'imageUrl': imageUrl,
        'timestamp': Timestamp.fromDate(now),
        'readBy': [_currentUserId!],
  ...
  Future<void> _sendSystemMessage(String conversationId, String content) async {
    try {
      final messageData = {
        ...
        'senderName': 'System',
        'senderRole': 'system',
        'type': msg.MessageType.system.toString(),
        'content': content,
    ...
    _typingListener = _firestore
      ...
      (snapshot) {
        final indicators = snapshot.docs
            .map((doc) => msg.TypingIndicator.fromFirestore(doc))
            .where((indicator) => 
